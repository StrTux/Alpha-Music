...
export const fallbackSongs = [
  {
    ...
    image: {
      uri: 'http://c.saavncdn.com/679/Thunderclouds-English-2018-20180809032729-500x500.jpg',
    },
    url: 'http://preview.saavncdn.com/679/6hiekPqnKvxKey2mNNO3fNk8NrPXHqyAfolWfOZ_96_p.mp4',
    artwork:
      'http://c.saavncdn.com/679/Thunderclouds-English-2018-20180809032729-500x500.jpg',
    duration: 187,
    album: 'Thunderclouds',
    albumId: '13615087',
  },
  ...
  try {
    ...
    if (song.id) {
      ...
      if (Array.isArray(song.image)) {
        ...
        const highQualityImg = song.image.find(
          img => img?.quality === '500x500' || img?.link?.includes('500x500'),
        );
        const mediumQualityImg = song.image.find(
          img => img?.quality === '150x150' || img?.link?.includes('150x150'),
        );
        imageUrl =
          highQualityImg?.link || mediumQualityImg?.link || song.image[0]?.link;
      ...
      // Validate image URL
      const validImageUrl =
        validateUrl(imageUrl) || 'https://via.placeholder.com/500';
      ...
      if (song.streamingUrl) {
        ...
        if (typeof song.downloadUrl[0] === 'object') {
          const highQuality = song.downloadUrl.find(
            url => url?.quality === '320kbps' || url?.quality === '160kbps',
          );
      ...
      const songData = {
        ...
        artwork:
          typeof validImageUrl === 'string'
            ? validImageUrl
            : validImageUrl.uri || 'https://via.placeholder.com/500',
...
export const fetchTrendingSongs = async (limit = 20) => {
  try {
    ...
    console.log('No valid songs found, using fallback songs');
    return fallbackSongs;
  } catch (error) {
  ...
  try {
    ...
    if (response.data && response.data.albums) {
      ...
      const albumPromises = response.data.albums.slice(0, limit).map(album =>
        apiService.getAlbumDetails(album.id).catch(err => {
      ...
      if (convertedSongs.length > 0) {
        ...
        console.warn(
          'No valid songs found in new releases, using fallback songs',
        );
        return fallbackSongs;
      }
    }
    ...
    console.log(
      'No albums found in new releases response, using fallback songs',
    );
    return fallbackSongs;
  } catch (error) {
