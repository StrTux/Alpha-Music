...
const PlaylistScreen = () => {
  ...
  // Find the index of the currently playing VLC track
  const getCurrentVlcIndex = () => {
    if (!vlcTrack) return -1;
    return songs.findIndex(
      s => s.name === vlcTrack.title && (s.primary_artists === vlcTrack.artist || s.subtitle === vlcTrack.artist)
    );
  };

  // Play the next song in the list
  const handlePlayNextVlc = async () => {
    const idx = getCurrentVlcIndex();
    if (idx >= 0 && idx < songs.length - 1) {
      await handlePlaySong(songs[idx + 1]);
    } else {
      setVlcTrack(null); // End of playlist
    }
  };

  const renderSong = ({ item, index }) => (
    <TouchableOpacity style={styles.songItem} onPress={() => handlePlaySong(item)} disabled={loadingId === item.id}>
  ...
  return (
    <LinearGradient colors={gradientColors} style={styles.gradient}>
      ...
      <View style={styles.header}>
        <TouchableOpacity onPress={() => nav.goBack()}><Icon name="arrow-back" size={24} color="#fff" /></TouchableOpacity>
        <View style={{ flex: 1 }} />
      ...
      <FlatList
        ...
        keyExtractor={i => i.id}
        contentContainerStyle={{ paddingBottom: vlcTrack ? 90 : 20 }}
        style={styles.list}
      />
      ...
      {vlcTrack && (
        <View style={styles.vlcContainer}>
          <MiniVlcPlayer track={vlcTrack} onClose={() => setVlcTrack(null)} onNext={handlePlayNextVlc} />
        </View>
      )}
    </LinearGradient>
  );
...
const styles = StyleSheet.create({
  ...
  songArtist: { color: '#ccc', fontSize: 14 },
  songDur: { color: '#ccc', fontSize: 14, marginRight: 12 },
  moreBtn: { padding: 8 },
  vlcContainer: { position: 'absolute', left: 0, right: 0, bottom: 0 },
});
