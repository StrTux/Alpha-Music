...
describe("Song API Tests", () => {
  describe("Song Details Endpoint", () => {
    ...
    test("GET song details for FyIoRUdgRHk should return valid data", async () => {
      const songId = "FyIoRUdgRHk"; // New song ID
      const response = await fetch(`${BASE_URL}/song/${songId}`);
      expect(response.status).toBe(200);
      ...
      await saveResponseToFile("test/responses/song_fyio.json", data);
