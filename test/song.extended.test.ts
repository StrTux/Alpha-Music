...
describe("Song Extended Tests", () => {
  ...
  test("GET /song?id=5WXAlMNt | Verify complete song metadata", async () => {
    const response = await app.request(`${BASE_URL}/song?id=5WXAlMNt`);
  ...
  test("GET /song?id=5WXAlMNt,9BjJPi0Y,IhKbmgyP | Multiple songs fetch", async () => {
    const response = await app.request(`${BASE_URL}/song?id=5WXAlMNt,9BjJPi0Y,IhKbmgyP`);
    expect(response.status).toBe(200);
  ...
  test("GET /song?link=invalid-url | Invalid song URL", async () => {
    const response = await app.request(`${BASE_URL}/song?link=invalid-url`);
  ...
  test("GET /song?link=https://example.com/song | Non-JioSaavn URL", async () => {
    const response = await app.request(`${BASE_URL}/song?link=https://example.com/song`);
    expect(response.status).toBe(400);
  ...
  test("GET /song/recommend?id=5WXAlMNt&limit=5 | Limited recommendations", async () => {
    const response = await app.request(`${BASE_URL}/song/recommend?id=5WXAlMNt&limit=5`);
    expect(response.status).toBe(200);
  ...
  test("GET /song?id= | Empty song ID", async () => {
    const response = await app.request(`${BASE_URL}/song?id=`);
    expect(response.status).toBe(400);
